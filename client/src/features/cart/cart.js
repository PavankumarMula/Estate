...
export default function Cart() {
  ...
  const totalAmount = items.reduce((amount, currentItem) => {
    return currentItem.product.price * currentItem.quantity + amount;
  }, 0);

  const totalQuantity = items.reduce((totalitems, currentItem) => {
    return currentItem.quantity + totalitems;
  }, 0);

  // quantity select handler
  const qtySelectHandler = (e, item) => {
    const qty = e.target.value;
    const updateItem = { ...item, quantity: +qty };
    ...
    <>
      <div>
        <div className="mx-auto mt-12 bg-white max-w-7xl px-4 sm:px-6 lg:px-8">
                      ...
                      <div className="ml-4 flex flex-1 flex-col">
                        ...
                        <div className="flex flex-1 items-end justify-between text-sm">
                          <div className="text-gray-500">
                            ...
                            <select onChange={(e) => qtySelectHandler(e, item)}>
          ...
          <div className="border-t border-gray-200 px-4 py-6 sm:px-6">
            <div className="flex justify-between text-base font-medium text-gray-900">
              <p>Subtotal of {totalQuantity} products</p>
              <p>$ {totalAmount}</p>
            </div>
